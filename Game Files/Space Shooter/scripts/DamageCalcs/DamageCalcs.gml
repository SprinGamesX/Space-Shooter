...
function AdditionalDamage(_enemy, _ship, _scale, _dmg_type, _stat = STAT.ATK){
	if (instance_exists(_enemy)){
		
		var _base_stat = 0;
		
		switch(_stat){
			case STAT.ATK: _base_stat = _ship.getATK(); break;
			case STAT.HP: _base_stat = _ship.getHP(); break;
			case STAT.DEF: _base_stat = _ship.getDEF(); break;
		}
		
		var _basedmg = (_base_stat) * _scale;
...
function AdditionalBreakDamage(_enemy, _ship, _scale){
	if (instance_exists(_enemy)){
		// Base dmg for break is 5% of the enemies HP 
		var _basedmg = (1 + _ship.getStatBonus(STAT.BREAKDMG)) * (_enemy.getHP() * 0.05) * _scale;
		...
		// 1 - Lvl multiplier
		var _damage = _basedmg * _res * _def * (1 - ((_enemy.lvl - _ship.lvl) * 0.01));
	
		CreateDamageIndicator(_enemy.x + random_range(0, 48), _enemy.y - random_range(-64, -16), string(round(_damage)) + "-BREAK!", _ship.element, !object_is_ancestor(_enemy.object_index, oEnemyElite) ? 0.5 : 1);
		_enemy.onHit(_damage, self);
	}
}